...
```{r load-packages}
library(ellmer)  # TODO: run renv::install("ellmer@0.4.0") if you get an error here
...
2. Download the `hf_api_key.txt` file attached to my message and open it on your computer in text editor
3. Open your **R project** in RStudio by running `usethis::edit_r_environ(scope = "project")`.
4. copy the content from `hf_api_key.txt` in the file that just opened in RStudio.
5. Save the file and **restart R** (Session -> Restart R in RStudio)
6. Verify that this succeeded by running the line
   ```r
...
::: {.callout-note title="cloud usage"}
...
::: {.callout-warning title="Costs"}

Generating responses with an LLM with the API token costs money. 
This money is charged on my account. 
I will cover the costs arising for your LLM usage during this course (up to $4 per person).

So please **be careful when using LLM in R with my API token**.
In particular:

1. Test your LLM text classification code always only with a handful of texts.
2. Avoid just clicking on "Run all" all the time and execute relevant lines selectively in your R scripts.
3. If running a code line or block that triggers an LLM API requests (e.g., `model$chat()`) results in an error, first make sure that you understand the source of the error instead of trying to run it again and again.

Should I see that someone uses the API token excessively, I'll ultimately need to revoke their access to it.


:::

::: {.callout-note title="Future use"}
...
We'll use `gpt-oss-20b` by OpenAI from inference provider `together`:
...
You find a list of available LLMs served through Hugging Face's inference providers API [here](https://huggingface.co/inference/models)

The following models deliver good cost effectiveness in my experience:

- `google/gemma-4-26B-A4B-it:deepinfra`
- `openai/gpt-oss-120b:novita`
- `Qwen/Qwen3.6-35B-A3B:deepinfra`
- `mistralai/Mixtral-8x22B-Instruct-v0.1:nscale`
...
```{r basic-prompting}
model$chat("What is the capital of Austria? Respond in one sentence.")
```
...
::: {.callout-warning title="Always keep the convo tidied up"}

You need to always reset the conversation history of the LLM starting a new prompt session.
Otherwise, earlier "turns" between user input and model response will influence the models classification decisions
...
```{r}
# NOTE: reset conversation history to start with an "empty" conversation
model$set_turns(list())
```
...
2. Use the `huggingface_chat` function and the ID of an available LLM (see [here](https://huggingface.co/inference/models)) to create a `Chat` object that allows you "talking" to the chosen LLM
3. Reset the conversation history before starting a new classification task using  `model$set_turns(list())`.
4. Define clear instructions for the classification task and response behavior using either a system message or user message.
5. Define a response format that constrains at least the allowed categories.
6. Use `parallel_chat_structured()` to classify multiple texts applying the same instructions.
